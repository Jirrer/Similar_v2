    ...
    public partial class MainFrame : Form
    {
        ...
        private async void PlaylistBox_KeyDown(object? sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                Runtime.PythonDLL = @"C:\Users\jrirr\AppData\Local\Programs\Python\Python313\python313.dll";
                ...
                using (Py.GIL())
                {
                    ...
                    string scriptPath = Path.GetFullPath(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, @"..\..\..\src"));
                    sys.path.append(scriptPath);

                    dynamic pythonScript = Py.Import("Similar");
                    dynamic result = pythonScript.GET_PLAYLIST();

                    // Convert result (Python list) to string[]
                    var songList = new List<string>();
                    foreach (PyObject item in result)
                    {
                        songList.Add(item.ToString());
                    }
